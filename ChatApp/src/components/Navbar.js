    ...
    <AppBar position='static' sx={{ background: '#18181a', borderBottom: 1, borderColor: '#999b9d' }} elevation={0}>
      <Toolbar>
        <IconButton size='large' edge='start' color='inherit'>
          <QuestionAnswerIcon sx={{ color: '#00ff66' }} />
        </IconButton>
        {isSmallRes ? (
          <></>
        ) : (
          <Typography variant='h6' component='div'>
            <div className='MyFont'>CHATAPP</div>
          </Typography>
        )}
        {props.user ? (
          <Stack direction='row' spacing={2} marginLeft='auto' alignItems='center'>
            <Avatar sx={{ bgcolor: '#00ff66', color: 'black' }}><div className='MyFont'>{props.user.username.charAt(0).toUpperCase()}</div></Avatar>
